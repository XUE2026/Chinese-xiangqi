...
async def create_temp_credential_endpoint(body: dict, current_user: User = Depends(get_current_admin)):
    ...
    user = await create_user(account_name=account_name, password_hash=password_hash, temp_uuid=temp_uuid_val, role=role, is_active=True)
    await create_temp_credential(temp_uuid=temp_uuid_val, login_id=account_name, password_hash=password_hash, otp_code_hash=otp_hash, expires_at=expires_at, is_used=False)

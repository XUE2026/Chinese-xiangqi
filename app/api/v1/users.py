...
async def list_users(current_user: User = Depends(get_current_super_admin)):
    ...
    return [{"id": u.id, "user_id": u.user_id, "account_name": u.account_name, "role": u.role, "signature": u.signature, "css_style": u.css_style, "is_active": u.is_active, "wins_total": u.wins_total} for u in users]
...
async def modify_user_id(user_id_field: str, body: dict, current_user: User = Depends(get_current_super_admin)):
    ...
    return {"status": "ok", "user": {"id": updated.id, "user_id": updated.user_id, "account_name": updated.account_name}}

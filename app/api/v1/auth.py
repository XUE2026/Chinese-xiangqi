...
from app.database.crud import create_session, get_temp_credential, get_user_by_account, mark_temp_credential_used, update_user
...
async def login(request: Request):
    ...
    if user:
        ...
        async with async_session() as session:
            result = await session.execute(select(User).where(User.temp_uuid == temp_cred.temp_uuid))
    ...
    return {"status": "ok", "access_token": token, "token_type": "bearer", "load_pages": load_pages, "resource_url": "/static/manifest.json"}

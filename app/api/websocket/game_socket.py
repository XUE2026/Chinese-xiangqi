...
class GameNamespace(socketio.AsyncNamespace):
    async def on_connect(self, sid, environ):
        ...
        if not user:
            await self.disconnect(sid)
            return False
        connected_users[sid] = {"user_id": user.id, "account_name": user.account_name, "role": user.role}
        return True
    ...
    async def on_move_piece(self, sid, data):
        ...
        await create_game_record(game_id=game_id, user_id=user_data["user_id"], move_from=from_pos_str, move_to=to_pos_str, fen_before=game.board_fen, fen_after=new_fen, is_drag=False)
        other_player = None
        ...
        await self.emit("move_made", {"game_id": game_id, "user_id": user_data["user_id"], "from": list(from_pos), "to": list(to_pos), "new_fen": new_fen, "next_turn": new_turn, "is_check": check_status, "is_checkmate": checkmate_status, "winner": winner}, room=room)
        return {"status": "ok"}
    ...
    async def on_admin_drag_piece(self, sid, data):
        ...
        from_pos_str = f"{from_pos[0]},{from_pos[1]}"
        to_pos_str = f"{to_pos[0]},{to_pos[1]}"
        await create_game_record(game_id=game_id, user_id=user_data["user_id"], move_from=from_pos_str, move_to=to_pos_str, fen_before=game.board_fen, fen_after=new_fen, is_drag=True)
        await update_game(game_id, board_fen=new_fen)
        room = f"game_{game_id}"
        await self.emit("move_made", {"game_id": game_id, "user_id": user_data["user_id"], "from": list(from_pos), "to": list(to_pos), "new_fen": new_fen, "is_drag": True}, room=room)
        return {"status": "ok"}
